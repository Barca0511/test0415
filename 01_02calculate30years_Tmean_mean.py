...
# 输出文件
output_file = os.path.join(input_folder, "ERA5Land_1981_2010_summer_tmean_mean30_by_diming_mean.csv")
...
for file in file_list:
    ...
    # 检查字段
    required_cols = {"region_name", "date", "temp_mean_c"}
    ...
    # 删除无效日期或无效温度
    df = df.dropna(subset=["date", "temp_mean_c"])
    ...
    yearly_mean = (
        summer_df
        .groupby(["region_name", "year"], as_index=False)["temp_mean_c"]
        .mean()
        .rename(columns={"temp_mean_c": "summer_mean"})
    )
...
result = (
    ...
    .mean()
    .rename(columns={"summer_mean": "temp_mean_mean30"})
)
...
# 调整列顺序
result = result[["region_name", "date", "temp_mean_mean30"]]
